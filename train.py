...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_set,
        ...
        worker_init_fn=seed_worker, prefetch_factor=2, persistent_workers=False)
...
def train(train_loader, model, optimizer, epoch, loss_scale, scaler):
    ...
    for i, (data_batch, label_batch) in enumerate(train_loader):
        ...
        for j in range(len(label_splits)):
            ...
            if j == 0:
                optimizer.zero_grad()
                if 'momentum_buffer' not in optimizer.state[optimizer.param_groups[-1]["params"][0]]:
                    optimizer.state[optimizer.param_groups[-1]["params"][0]]["momentum_buffer"] = torch.zeros_like(optimizer.param_groups[-1]["params"][0])
